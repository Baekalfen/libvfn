...
static int nvme_configure_cq(struct nvme_ctrl *ctrl, int qid, int qsize, int vector)
{
	...
	*cq = (struct nvme_cq) {
	...
	if (vfio_map_vaddr(&ctrl->pci, cq->vaddr, len, &cq->iova)) {
...
static void nvme_discard_cq(struct nvme_ctrl *ctrl, struct nvme_cq *cq)
{
	...
	if (vfio_unmap_vaddr(&ctrl->pci, cq->vaddr, NULL))
			     ...
			     struct nvme_cq *cq, unsigned long UNUSED flags)
{
	...
	*sq = (struct nvme_sq) {
	...
	len = pgmapn(&sq->pages.vaddr, qsize, 1ull << ctrl->config.mps);

	if (len < 0)
		return -1;

	if (vfio_map_vaddr(&ctrl->pci, sq->pages.vaddr, len, &sq->pages.iova)) {
		log_debug("failed to map vaddr\n");
		goto unmap_pages;
	}
	...
	if (vfio_map_vaddr(&ctrl->pci, sq->vaddr, len, &sq->iova)) {
		log_debug("failed to map vaddr\n");
		goto unmap_sq;
	}
...
free_sq_rqs:
	free(sq->rqs);
unmap_pages:
	if (vfio_unmap_vaddr(&ctrl->pci, sq->pages.vaddr, NULL))
		log_debug("failed to unmap vaddr\n");

	pgunmap(sq->pages.vaddr, (size_t)sq->qsize << ctrl->config.mps);
...
static void nvme_discard_sq(struct nvme_ctrl *ctrl, struct nvme_sq *sq)
{
	...
	if (vfio_unmap_vaddr(&ctrl->pci, sq->vaddr, NULL))
		log_debug("failed to unmap vaddr\n");
...
int nvme_enable(struct nvme_ctrl *ctrl)
{
	...
	cc =
		NVME_FIELD_SET(ctrl->config.mps - 12, CC_MPS) |
...
static int nvme_init_dbconfig(struct nvme_ctrl *ctrl)
{
	...
	if (vfio_map_vaddr(&ctrl->pci, ctrl->dbbuf.doorbells, __VFN_PAGESIZE, &prp1))
		return -1;

	if (pgmap((void **)&ctrl->dbbuf.eventidxs, __VFN_PAGESIZE) < 0)
		return -1;

	if (vfio_map_vaddr(&ctrl->pci, ctrl->dbbuf.eventidxs, __VFN_PAGESIZE, &prp2))
		return -1;
...
int nvme_init(struct nvme_ctrl *ctrl, const char *bdf, const struct nvme_ctrl_opts *opts)
{
	...
	if ((12 + mpsmax) < __VFN_PAGESHIFT) {
		log_debug("nvme_init: controller maximum page size is smaller than native page size. Capping page size to %llu\n", 1ull << (12u + mpsmax));
		ctrl->config.mps = 12 + mpsmax;
	}
